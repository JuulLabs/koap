...
internal fun Option.toFormat(): Format =
    when (val option = this) {
        ...
        is QBlock1 -> uint(19, Option.numMSzx(option.blockNumber, option.more, option.blockSize))
        is LocationQuery -> string(20, option.uri)
        is Block2 -> uint(23, Option.numMSzx(option.blockNumber, option.more, option.blockSize))
        is Block1 -> uint(27, Option.numMSzx(option.blockNumber, option.more, option.blockSize))
        is Size2 -> uint(28, option.bytes)
        is QBlock2 -> uint(31, Option.numMSzx(option.blockNumber, option.more, option.blockSize))
